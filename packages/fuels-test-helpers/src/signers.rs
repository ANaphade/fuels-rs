...
pub async fn launch_provider_and_get_single_wallet(node_config: Config) -> LocalWallet {
    let mut wallets =
        launch_provider_and_get_wallets(WalletsConfig::new_single(None, None), node_config).await;
...
pub async fn launch_provider_and_get_wallets(
    ...
    let (provider, _) = setup_test_provider(all_coins, node_config).await;
...
mod tests {
    ...
    async fn test_wallet_config() {
        ...
        let wallets = launch_provider_and_get_wallets(config, Config::local_node()).await;
